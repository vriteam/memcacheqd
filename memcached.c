...
#define KEY_TOKEN 1
//2 is queue_add + id,that is queue_add_0_
#define QUEUE_KEY_TOKEN (KEY_TOKEN + 3)
#define MAX_TOKENS (8 + 3)

#define SUBCONTEXT_TOKEN 1
#define SUBCONTEXT_COMMAND_TOKEN 2
#define SUBCONTEXT_PARAM_TOKEN 3
typedef struct token_s {
    char *value;
    size_t length;
} token_t;
static size_t tokenize_command(char *, token_t *, const size_t );
...
static void settings_init(void) {
    ...
    settings.max_q_size = 1024 * 1024; /* The famous 1MB upper limit. */
    settings.max_queues = QUEUE_NUMS_MAX; /* . */
}
...
static void complete_incr_bin(conn *c) {
    ...
    if (it && (c->binary_header.request.cas == 0 ||
        ...
        size_t ntokens; 
        ntokens = tokenize_command(key, tokens, MAX_TOKENS);
        if (ntokens != 5) {
                write_bin_error(c, PROTOCOL_BINARY_RESPONSE_ENOMEM, 0);
        }
        key  = tokens[QUEUE_KEY_TOKEN - 1].value;
        nkey = tokens[QUEUE_KEY_TOKEN - 1].length;

        if ( !safe_strtoul(tokens[SUBCONTEXT_PARAM_TOKEN - 1].value, &qid) ) {
            write_bin_error(c, PROTOCOL_BINARY_RESPONSE_NOT_STORED, 0);
            return;
        } else {
            /* Save some room for the response */
            rsp->message.body.value = htonll(req->message.body.initial);
            it = item_alloc(qid, key, nkey, 0, realtime(req->message.body.expiration),
...
static void process_bin_sasl_auth(conn *c) {
    ...
    item *it = item_alloc(qid, key, nkey, 0, 0, vlen);
...
static void process_bin_update(conn *c) {
    ...
    it = item_alloc(QUEUE_ID_UNKNOWN, key, nkey, req->message.body.flags,
...
static void process_bin_append_prepend(conn *c) {
    ...
    it = item_alloc(QUEUE_ID_UNKNOWN, key, nkey, 0, 0, vlen+2);
        ...
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) {
            ...
            if (stored == NOT_STORED) {
                ...
                new_it = do_item_alloc(it->qid, key, it->nkey, flags, old_it->exptime, it->nbytes + old_it->nbytes - 2 /* CRLF */);
...
static void server_stats(ADD_STAT add_stats, conn *c) {
    ...
    STATS_UNLOCK();
    memcache_queue_stats(add_stats, c);
}
...
static void process_stat(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    if (ntokens == 2) {
        ...
        process_stat_settings(&append_stats, c);
    } else if ( ntokens >= 4 && strcmp(subcommand, QUEUE_RESERVE_CONTEXT) == 0) {
        if ( strcmp(tokens[2].value, QUEUE_RESERVE_KEY_STATALL) == 0 ) {
            queues_stats(&append_stats, c);
        } else if ( ntokens == 5 && strcmp(tokens[2].value, QUEUE_RESERVE_KEY_STAT ) == 0 ) {
            uint32_t qid;
            if( safe_strtoul(tokens[3].value, &qid) && 
                queue_exists((size_t)qid) )
                queue_stat(&append_stats, c, (size_t)qid);
            else
                return;
        }
...
static inline void process_queue_get_command(conn *c, token_t *tokens, size_t ntokens, bool fetch) {
    int i = 0;
    item *it;
    //所有key的指针
    assert(c != NULL);
    ...
    uint32_t qid;
    if ( !safe_strtoul(tokens[SUBCONTEXT_PARAM_TOKEN].value, &qid) ||
            !queue_exists((size_t)qid) ) {
        out_string(c, "Invalid queue id");
        return;
    }
    if(fetch)
        it = item_queue_get((size_t)qid);
    else
        it = item_queue_look((size_t)qid);
    //获得到了对应的key的数据
    if (it) {
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    if ( ntokens == 9 || ntokens == 10 ) {
        if (tokens[QUEUE_KEY_TOKEN].length > KEY_MAX_LENGTH) {
            out_string(c, "CLIENT_ERROR bad command line format");
            return;
        }

        if( strcmp(tokens[SUBCONTEXT_COMMAND_TOKEN].value, QUEUE_RESERVE_KEY_RADD) == 0 ) {
            key = rnd;
            nkey = sprintf(key, "%lld", (unsigned long long int)get_rand_key());
        } else {
            key = tokens[QUEUE_KEY_TOKEN].value;
            nkey = tokens[QUEUE_KEY_TOKEN].length;
        }
        if (! safe_strtoul(tokens[SUBCONTEXT_PARAM_TOKEN].value, &qid) ) {
            out_string(c, "CLIENT_ERROR bad command line format");
            return;
        }
        qid = queue_get_id(qid);
        if (! (safe_strtoul(tokens[5].value, (uint32_t *)&flags)
               && safe_strtol(tokens[6].value, &exptime_int)
               && safe_strtol(tokens[7].value, (int32_t *)&vlen))) {
            out_string(c, "CLIENT_ERROR bad command line format");
            return;
        }
        // does cas value exist?
        if (handle_cas) {
            if (!safe_strtoull(tokens[8].value, &req_cas_id)) {
                out_string(c, "CLIENT_ERROR bad command line format");
                return;
            }
        }
    ...
    it = item_alloc(qid, key, nkey, flags, realtime(exptime), vlen);
...
enum delta_result_type do_add_delta(conn *c, item *it, const bool incr,
    ...
    if (res + 2 > it->nbytes) { /* need to realloc */
        item *new_it;
        new_it = do_item_alloc(it->qid, ITEM_key(it), it->nkey, atoi(ITEM_suffix(it) + 1), it->exptime, res + 2 );
        if (new_it == 0) {
            return EOM;
        }
...
static void process_command(conn *c, char *command) {
    ...
    if (ntokens >= 3 &&
        ...
        if(ntokens == 5 && strcmp( tokens[SUBCONTEXT_TOKEN].value, QUEUE_RESERVE_CONTEXT) == 0) {
            if(strcmp( tokens[SUBCONTEXT_COMMAND_TOKEN].value, QUEUE_RESERVE_KEY_GET) == 0)
                process_queue_get_command(c, tokens, ntokens, true);
            else if(strcmp( tokens[SUBCONTEXT_COMMAND_TOKEN].value, QUEUE_RESERVE_KEY_LOOK) == 0)
                process_queue_get_command(c, tokens, ntokens, false);
            else
            ...
            (ntokens == 9 || ntokens == 10) &&
               (
                (strcmp(tokens[COMMAND_TOKEN].value, "add") == 0 && (comm = NREAD_ADD) &&
                 strcmp(tokens[SUBCONTEXT_TOKEN].value, QUEUE_RESERVE_CONTEXT) == 0 &&
                 ( strcmp(tokens[SUBCONTEXT_COMMAND_TOKEN].value, QUEUE_RESERVE_KEY_ADD) == 0 || 
                  strcmp(tokens[SUBCONTEXT_COMMAND_TOKEN].value, QUEUE_RESERVE_KEY_RADD) == 0) 
                )
               ) ) {
    ...
    } else if (ntokens >= 2 && ntokens <= 4 && (strcmp(tokens[COMMAND_TOKEN].value, "flushall") == 0)) {
        time_t exptime = 0;
        set_current_time();
...
#endif
    printf("-Q            Size of queue in m, default is 1m\n");
    printf("-M            Num of queues, default and max is %d\n", QUEUE_NUMS_MAX);
    return;
}
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
          ...
          "I:"  /* Max item size */
          "S"   /* Sasl ON */
          "Q:"  /* max size for queue in m */
          "M:"  /* max nums of queues*/
        ))) {
        switch (c) {
        case 'a':
        ...
        case 'Q':
            ...
            if (settings.max_q_size == 0) {
                fprintf(stderr, "queue size must be greater than 0\n");
                return 1;
            }
            break;
        case 'M':
            settings.max_queues = atoi(optarg);
            if (settings.max_queues == 0 || settings.max_queues > QUEUE_NUMS_MAX) {
                fprintf(stderr, "queues num must be greater than 0 and less then %d\n", QUEUE_NUMS_MAX);
                return 1;
            }
            break;
        default:
    ...
    queues_init(settings.max_q_size, settings.max_queues);
    /*
