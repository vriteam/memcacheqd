...
static pthread_mutex_t anonymous_id_lock = PTHREAD_MUTEX_INITIALIZER;
 ...
 */
item *item_alloc(uint32_t qid, char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes) {
    item *it;
    pthread_mutex_lock(&cache_lock);
    it = do_item_alloc(qid, key, nkey, flags, exptime, nbytes);
...
item *item_queue_get(size_t qid) {
    item *it = NULL;
    pthread_mutex_lock(&cache_lock);
    it = (item*)queue_get(true, qid);
    if(it != NULL){
        it->refcount++;
        do_item_unlink(it);
    }
...
item *item_queue_look(size_t qid) {
    item *it = NULL;
    pthread_mutex_lock(&cache_lock);
    it = (item*)queue_get(false, qid);
    if(it != NULL){
        it->refcount++;
    }
    pthread_mutex_unlock(&cache_lock);
...
enum store_item_type store_item(item *item, int comm, conn* c) {
    ...
    pthread_mutex_lock(&cache_lock);
    if( comm == NREAD_ADD && queue_full(item->qid) ) {
        queue_plus_otimes(item->qid);
    }else if( (ret = do_store_item(item, comm, c) ) == STORED ) {
#ifdef ENABLE_UPDATE
        item->qptr = queue_add(item, item->qid);
#else
        queue_add(item, item->qid);
#endif
#ifdef ENABLE_UPDATE
    }else if( ret == UPDATED ) {
        ret = STORED;
...
void memcache_queue_stat_reset(void) {
    pthread_mutex_lock(&cache_lock);
    queue_stat_reset_all();
    pthread_mutex_unlock(&cache_lock);
}
void memcache_queue_stats(ADD_STAT add_stats, void *c) {
    pthread_mutex_lock(&cache_lock);
    queue_stats(add_stats, c);
    pthread_mutex_unlock(&cache_lock);
}
void memcache_queues_stats(ADD_STAT add_stats, void *c) {
    pthread_mutex_lock(&cache_lock);
    queues_stats(add_stats, c);
    pthread_mutex_unlock(&cache_lock);
}

uint64_t get_rand_key(void){
    uint64_t rk;
    pthread_mutex_lock(&anonymous_id_lock);
    rk = anonymous_id == UINT64_MAX ? 0 : anonymous_id++;
    pthread_mutex_unlock(&anonymous_id_lock);
    return rk;
}
